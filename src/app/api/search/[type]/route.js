import { getRealAlbum, getSearchAlbum, getSongData } from '@/app/controllers/MusicController'
...
export async function POST(req, res) {
    ...
    const CACHE_KEY = 'search/' + type + SongId;
    try {
        const cachedData = await getFromCache(CACHE_KEY)
        if (cachedData) {
            return NextResponse.json({ cached: "cacheddata", data: cachedData }, { status: 200 })
        }
        const data = await getSearchAlbum(SongId);
        const freshData = await getRealAlbum(freshData?.id);
